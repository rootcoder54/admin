...
export function FilterDateIntervention<TData>({
  table
}: DataTableToolbarProps<TData>) {
  const filterValue = table.getColumn("createdAt")?.getFilterValue() as
  ...
  React.useEffect(() => {
    if (dateFilter[0] !== undefined || dateFilter[1] !== undefined) {
      table.getColumn("createdAt")?.setFilterValue(dateFilter);
    }
  }, [dateFilter, table]);
